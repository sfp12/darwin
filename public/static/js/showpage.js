...
jQuery(function($){
    ...
    jst.getData = function(){
        ...
        $.get('/indexajax/getIndexData', {"userId": hsd.user_id}, function(data){            
            ...
            if(files.length === +files.length){
                ...
                jst.createFilesTree('#tree-files', files);
                jst.createResultsTree('#tree-results', results_data);
                jst.createGraphsTree('#tree-graphs', graphs_data);

                hsd.time = null;
                // data table双击 打开table          
                $('#tree-files').on('dblclick','.jstree-anchor', function (e) {
                ...
                $('#tree-files').on('click', '.jstree-anchor', function (e) {
                    var instance = $.jstree.reference(this),
                        node = instance.get_node(this);
                    clearTimeout(hsd.time);
                    hsd.time = setTimeout(function(){
                        //单击事件在这里                        
                        //result的容器
                        var con_r = $('#tree-results');
                        //graphs的容器
                        var con_g = $('#tree-graphs');
                        //选中元素的序号
                        var index = node.id.replace('table','');

                        //取消data table高亮               
                        $('#'+$('#tree-files').jstree('get_selected')).parent().find('li.active').toggleClass('active');
                        //data table中选中的li高亮
                        $('#'+$('#tree-files').jstree('get_selected')).toggleClass('active');
                ...
                $('#tree-results').on('click', '.jstree-anchor', function(e){                    
                    if(e.which === 1){
                ...
                // result 左键 跳转到analysis
                $('#tree-graphs').on('click', '.jstree-anchor', function(e){                    
                    if(e.which === 1){
                        if($.jstree.reference(this).get_node(this)){
                            $('#flag li:eq(2) a').tab('show');
                            $('#analysis').text($.jstree.reference(this).get_node(this).id);
    ...
    jst.treeRefresh = function(){
        ...
        $.get('/indexajax/getIndexData', {"userId": hsd.user_id}, function(data){
            ...
            if(files.length === +files.length){
                ...
                $('#tree-files').jstree(true).settings.core.data = files;
                $('#tree-files').jstree(true).refresh();
                $('#tree-results').jstree(true).settings.core.data = results_data;
                $('#tree-results').jstree(true).refresh();
                $('#tree-graphs').jstree(true).settings.core.data = graphs_data;
                $('#tree-graphs').jstree(true).refresh();
            } 
    ...
    jst.init = function(){
        //delete modal 不删除节点
        $('#delete-no').unbind().bind('click', function(){
            $('#delete-modal').modal('hide');
        }); 

        //delete modal 删除节点
        $('#delete-yes').unbind().bind('click', function(){
            $('#delete-modal').modal('hide');
        ...
        //new 新建table
        $('#file-new').unbind().bind('click', function(){
        ...
        $('#upload-file-button').unbind().bind('click', function(e){

            // 获取文件名
            var file_name = $('#upload-file').val().split('\\').pop();
            var array_t = [];
            $('#tree-files a').each(function(index, ele){
            ...
            if($.inArray(file_name, array_t) === -1){
                ...
                var modal_s = $('#file-upload').attr('active') === undefined ? $('#data-upload') : $('#file-upload');
                ...
                $.ajaxFileUpload({
                    ...
                    success: function(data, status){
                        ...
                        if(data.status === 0){
                            $('#message-modal p').text('Successfully uploaded!');
                ...
                //重名
                $('#message-modal p').text('文件名重复，请修改文件名。');
                $('#message-modal').modal('show');
            }
        ...
        //export 下载table
        $('#file-export').unbind().bind('click', function(){

            if($('#'+$('#tree-files').jstree('get_selected')).length !== 0){

                // 更新<a>的href，并点击
                var index = +$('#'+$('#tree-files').jstree('get_selected')).attr('id').replace('table','');
                $('#file-export-a').attr('href','/file/download?dataId='+index+'&userId='+hsd.user_id);
                var a_array = $('#file-export-a');
                a_array[0].click();

            }else{

                $('#message-modal p').text('请选择文件。');
                $('#message-modal').modal('show');
            }
        });
    };
    ...
    jst.createFilesTree = function(con, data){
            ...
            'contextmenu' : {
                'items' : function(node) {
                    ...
                    tmp.Remove.action = function(b){
                        ...
                        $('#delete-modal p').text('Delete this file?');
                    ...
                    tmp.Export = {
                        ...
                        "action" : function(data){
                            //给<a>,的href赋值，再click
                            $('#file-export-a').attr('href','/file/download?dataId='+data.reference.prevObject.selector.replace('#table','')+'&userId='+hsd.user_id);
                            var a_array = $('#file-export-a');
                            a_array[0].click();                            
                        }
                    };                   
        ...
        .on('rename_node.jstree', function(e, data){ 
            ...
            // $('#tree-results').jstree('rename_node', '#result'+argu.dataId, data.text);            
            // $('#tree-graphs').jstree('rename_node', '#graphs'+argu.dataId, data.text);
            ...
            $.post('/indexajax/saveName/', argu, function(data){                
                $('#message-modal p').text(data.data);
                $('#message-modal').modal('show');
            }, 'json');
        })                
        ;
    ...
    hsd.getData = function(index){
        ...
        $.post('/indexajax/get', argu,  function(data){
            ...
            $('#data-export a').attr('href', '/file/download?dataId='+hsd.data_id+'&userId='+hsd.user_id); 
    ...
    hsd.pageInit = function(data){
        ...
        $('#filter-compare').html(str);

        //加载表格的名字
        $('#table-name').text(hsd.table_name);

        //新建表格
        $('#data-new').unbind().bind('click', function(){       
            if(!hsd.prev_table){                        
                if(hsd.modify){
                    $('#save-modal').modal();
                    $('#save-modal p').text('Save changes to this document?');
                    hsd.open_id = undefined;
                }else{
                    hsd.data = [[]];
        ...
        //不保存
        $('#save-no').unbind().bind('click', function(){
            $('#table-name-star').css('display', 'none');
        ...
        //保存
        $('#save-yes').unbind().bind('click', function(){
            $('#save-modal').modal('hide');
            $('#data-save').click();                      
        });

        //改变表格名字的modal 按钮的事件处理程序
        $('#data-modal .modal-footer button').unbind().bind('click', function(){

            $('#data-modal').modal('hide');

            var table_name = $('#table-name-save').val();
        ...
        //save按钮的事件
        $('#data-save').unbind().bind('click', function(){

            // 保存新建的表
            if(hsd.new_table){
                hsd.modal_flag = 1;
                $('.data-modal .modal-title').text('Save File');
                $('#data-modal').modal();
            }else{
                //保存已有表格
        ...
        //save as按钮的事件
        $('#data-saveas').unbind().bind('click', function(){
            //弹出modal，输入名字
        ...
        //log10
        $('#data-log10').unbind().bind('click', function(){
            hsd.logCal('#data-log10');
        });

        //log2
        $('#data-log2').unbind().bind('click', function(){
            hsd.logCal('#data-log2');
        });

        //√
        $('#data-sqrt').unbind().bind('click', function(){
            hsd.logCal('#data-sqrt');
        });

        //过滤
        $('#data-filter').unbind().bind('click', function(){
            hsd.filter('#data-filter');
        });
    };
    ...
    hsd.logCalOpe = function(argu){
        ...
        for(var i = 0, l = argu.data.length; i < l; i++){
            for(var j = 0, k = argu.data[i].length; j < k; j++){
                if(argu.origin_ele == '#data-log10'){
                    if(+argu.data[i][j] > 0){
                        argu.data[i][j] = Math.log(argu.data[i][j])/Math.LN10;
                    }
                }else if(argu.origin_ele == '#data-log2'){
                    if(+argu.data[i][j] > 0){
                        argu.data[i][j] = Math.log(argu.data[i][j])/Math.LN2;
                    }
                }else if(argu.origin_ele == '#data-sqrt'){
                    if(+argu.data[i][j] > 0){
    ...
    hsd.filter = function(con){
        ...
        argu.data.data = hsd.data;
        argu.data.origin_ele = con;
        argu.data.col = $('#filter-col').val();
        argu.data.compare = $('#filter-compare').val();
        argu.data.limitation =  $('#filter-limitation').val();
    ...
    hsd.webWorkersCon = function(argu){
        ...
        if(typeof(Worker) !== 'undefined'){
            ...
            w.onmessage = function(event){
                ...
                if(argu.data.origin_ele == '#data-log10' || argu.data.origin_ele == '#data-log2' || argu.data.origin_ele == '#data-sqrt'){
        ...
        hsd.hot = new Handsontable(hsd.con[0], {
            ...
            afterChange: function(array, event_name){
                if((array != null) && event_name != 'loadData' && array[0][2] != +array[0][3]){
                    hsd.modify = true;
                    $('#table-name-star').css('display', 'inline-block');
                }
            },
